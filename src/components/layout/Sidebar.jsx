...
export default function Sidebar({ collapsed, setCollapsed, userRole = 'institution_admin' }) {
  ...
  const { canView, fullAccess, loading: permissionsLoading } = usePermissions()
  ...
  // Filter menu items based on both role and dynamic permissions
  const filteredMenuItems = menuItems.filter(item => {
    // First check if role is allowed (legacy check)
    if (!item.roles.includes(userRole)) return false
    
    // Platform admins (super_admin, admin) bypass permission checks
    if (['super_admin', 'admin'].includes(userRole)) return true
    
    // If fullAccess is granted, show all
    if (fullAccess) return true
    
    // If item has a module, check if user can view that module
    if (item.module) {
      return canView(item.module)
    }
    
    // Default to showing if no module specified
    return true
  })
